      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 py-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* Ligar — desktop (mantém vermelho, compacto) */}
            <a
              href={telHref}
            ...
            {/* Consultoria Jurídica — mantém outline */}
            <Link
              href={`#${TARGET_ID}`}
              prefetch={false}
        ...
        {/* CTAs mobile: Ligar (idêntico ao Whats) + WhatsApp */}
        <div className="md:hidden mt-2.5 space-y-2.5">
          {/* Emergência? Ligue agora — mesmo layout do WhatsApp mobile, mas com tel: */}
          <a
            href={telHref}
            onClick={() => handleCallClick("header_ligar")}
            className="
              block w-full rounded-md bg-red-600
          ...
          >
            <span className="inline-flex items-center justify-center gap-1.5">
              {/* ícone telefone, mesmo tamanho do ícone do Whats */}
          ...
          {/* WhatsApp — mobile full width (inalterado no visual) */}
          <a
            href={whatsappHref}
            target="_blank"
            rel="noopener noreferrer"
            onClick={handleWhatsappClick}
            className="
              block w-full rounded-md bg-green-600
              px-4 py-2.5 text-sm sm:text-base font-bold text-white text-center
