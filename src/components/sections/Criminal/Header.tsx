...
export default function Header() {
  ...
  // ======== FONTE DA VERDADE DO TELEFONE (corrigido) ========
  const rawPhone = "+5548991447874"; // mantém +55 para o tel:
  const telHref = `tel:${rawPhone}`; // "tel:+5548991447874"

  // para dataLayer (só dígitos) e para aria-label (bonito)
  const phoneDigits = useMemo(() => rawPhone.replace(/\D/g, ""), [rawPhone]);
  ...
  const whatsappHref = useMemo(
    () => `https://wa.me/5548991447874?text=${encodeURIComponent(whatsappMessage)}`,
    []
  );
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-4 md:px-6 py-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* Ligar — visível apenas no desktop (no mobile vira CTA cheio igual ao WhatsApp lá embaixo) */}
            <a
              href={telHref}
              ...
              className="
                ...
                whitespace-nowrap
              "
              aria-label={`ligar para emergência em ${prettyPhone}`}
            >
            ...
            {/* WhatsApp — só desktop aqui (mobile vira CTA cheio lá embaixo) */}
            <a
              href={whatsappHref}
              target="_blank"
              ...
              className="
                ...
                whitespace-nowrap
              "
              aria-label="iniciar conversa de consulta no WhatsApp"
            >
        ...
        {/* linha 2: CTAs cheios abaixo (apenas mobile) */}
        {/* MOBILE: botão de Ligar com o mesmo visual do WhatsApp */}
        <a
          href={telHref}
          ...
          className="
            ...
            md:hidden
          "
          aria-label={`ligar agora para emergência em ${prettyPhone}`}
        >
        ...
        {/* MOBILE: botão WhatsApp cheio (original) */}
        <a
          href={whatsappHref}
          target="_blank"
          rel="noopener noreferrer"
          onClick={handleWhatsappClick}
          className="
            mt-2.5 block w-full rounded-md bg-green-600
            px-4 py-2.5 text-sm sm:text-base font-bold text-white text-center
